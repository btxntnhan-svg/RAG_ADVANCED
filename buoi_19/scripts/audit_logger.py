...
class AuditLogger:
    ...
    def log(self, **kwargs) -> dict:
        """Flexible log method accepting arbitrary keyword arguments."""
        user_id = kwargs.get("user_id", kwargs.get("user_id_demo", "SYSTEM"))
        user_role = kwargs.get("user_role", "Admin")
        if isinstance(user_role, list):
            user_role = user_role[0] if user_role else "Admin"
        action = kwargs.get("action", "GENERAL_ACTION")
        domain = kwargs.get("domain", "General")
        request_id = kwargs.get("request_id")
        status = kwargs.get("status", "SUCCESS")
        
        details = {k: v for k, v in kwargs.items() if k not in ["user_id", "user_id_demo", "user_role", "action", "domain", "request_id", "status"]}
