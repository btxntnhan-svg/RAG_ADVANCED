...
- Đầu vào: [`chunks_secure.csv`](file://C:\Users\Win10-64\Desktop\RAG_ADVANCED\buoi_14).
- Cấu hình API key được nạp từ tệp tin [`.env`](file:///C:\Users\Win10-64\Desktop\RAG_ADVANCED\\buoi_14\.env) cục bộ.
...
2. Viết tập lệnh `evaluate_rag_pipeline.py` thực hiện tuần tự các bước sau:
   ...
   c. Chạy Ragas đánh giá 4 metrics: Context Precision, Context Recall, Faithfulness, và Answer Relevancy:
      - Cấu hình LLM trọng tài (Judger) sử dụng `ChatOpenAI` gọi mô hình "openai/gpt-oss-20b:deepinfra" trỏ qua HF Router (base_url="https://router.huggingface.co/v1", api_key=os.environ["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"]), tắt reasoning của mô hình.
...
client = OpenAI(
    ...
    api_key=os.environ["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"],
)
